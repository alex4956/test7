...
def read_login():
    with open('PostgreSQL_login.txt') as f_inp:
        user_name = ''
        user_password = ''
        for line in f_inp:
            s = line.strip()
            ...
            if p > -1:
                user_name = s[11:-1]
            p = s.find('user_password="')
            if p > -1:
                user_password = s[15:-1]
        # print(user_name, user_password)
    return user_name, user_password


user_name, user_password = read_login()
print('user_name =', user_name, ',   user_password =', user_password)

connection = create_connection_postgresql(user_name, user_password)
...
def write_row14_3(row, f_out):
    ...
    while len(s) > 0:
        if len(s) > 80:
            ...
            f_out.write(s + "\n")
            break
    f_out.write('date = ' + row[4])
    f_out.write(', user = ' + str(row[5]) + ', flag_del = ' + str(row[6]) )
    f_out.write(', count = ' + str(row[7]) + ', view = ' + str(row[8]) + '\n')
    f_out.write('plus = ' + str(row[9]) + ', minus = ' + str(row[10]) + ', money = ' + str(row[11]))
    f_out.write(' flags = ' + str(row[12]) + ', date_r = ' + row[13] + '\n')
...
def delete_table_1():
    cursor.execute("DROP TABLE IF EXISTS str_articles")
    connection.commit()
    print('Таблица str_articles удалена из DB PostgreSQL')
    return
